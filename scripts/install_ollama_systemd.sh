#!/usr/bin/env bash
# scripts/install_ollama_systemd.sh — render and install the crew-chief Ollama
# container as a user-level systemd service via the shared clockwork scheduler.
#
# Usage:
#   scripts/install_ollama_systemd.sh [options]
#
...
TEMPLATE_PATH="${REPO_ROOT}/config/clockwork/crew-chief-ollama.toml.template"
...
usage() {
  cat <<EOF
Usage: install_ollama_systemd.sh [options]

Render the crew-chief Ollama container as a user-level systemd service via
clockwork and enable + start it by default.
...
chmod +x "${REPO_ROOT}/scripts/run_ollama.sh"
...
systemctl --user enable --now crew-chief-ollama.service
echo ""
systemctl --user status crew-chief-ollama.service --no-pager || true
